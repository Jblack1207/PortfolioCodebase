...
export default function CaseStudy({ project }: { project: Project }) {
  ...
  return (
    <>
      <ScrollToTop />
      <section className="mx-auto max-w-295 scroll-mt-20 px-6 pt-32 pb-16 sm:px-12">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={staggerContainer}>
          ...
          <motion.p variants={fadeUp} className="m-0 mb-8 max-w-[62ch] text-lg leading-[1.6] text-foreground/70">
            {extra?.tagline ?? project.shortDescription}
          </motion.p>
          <motion.div variants={fadeUp} className="mb-10 flex flex-wrap gap-1.5">
          ...
          <motion.div variants={fadeUp} className="flex flex-wrap gap-2.5">
            {project.liveUrl && (
              <a className="btn btn-primary px-5 py-2.5 text-sm" href={project.liveUrl} target="_blank" rel="noopener noreferrer">
                View live
              </a>
            )}
            {project.githubUrl && (
              <a className="btn btn-secondary px-5 py-2.5 text-sm" href={project.githubUrl} target="_blank" rel="noopener noreferrer">
                Source on GitHub
              </a>
            )}
      ...
      <section className="mx-auto max-w-295 px-6 py-14 sm:px-12">
        <motion.div
          initial="hidden"
          whileInView="visible"
      ...
      <section className="mx-auto max-w-295 px-6 pt-4 pb-24 sm:px-12">
        <div className="hr mb-10" />
        <div className="flex flex-wrap items-center justify-between gap-5">
          <p className="m-0 text-sm text-foreground/55">Thanks for reading — there&rsquo;s more where that came from.</p>
